...
def statelevel(y, n):
    ...
    # compute statelevels
    iLowerRegion = next(
        x[0] for x in enumerate(histogram) if x[1] > 0
    )  # find(histogram > 0, 1, 'first');
    histogram2 = histogram[::-1]
    iUpperRegion = (
        len(histogram2) - next(x[0] for x in enumerate(histogram2) if x[1] > 0) - 1
    )  # find(histogram > 0, 1, 'last');
