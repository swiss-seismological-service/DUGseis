...
class REC(object):
    ...
    def work(self):
        ...
        self.pick = self.wt.stats.starttime + self.wt.stats.delta * self.idx
    ...
    def get_rec_function(self):
        if isinstance(self.recfn, np.ndarray) and self.recfn.size > 0:
            ...
            logger.warning(
                "Missing EVALUATION FUNCTION! " + "Run the work method first!"
            )
    ...
    def get_pick_index(self):
        if self.idx:
            ...
            logger.warning("Missing INDEX! " + "Run the work method first!")
    ...
    def get_pick(self):
        if self.pick:
            ...
            logger.warning("Missing PICK! " + "Run the work method first!")
...
class AIC(object):
    ...
    def work(self):
        ...
        self.aicfn = np.zeros(self.wt.data.size - 1, dtype=np.float32, order="C")
        ret = myclib.aicp(tmparr, self.wt.data.size, self.aicfn, C.byref(pminidx))
        if ret != 0:
    ...
    def get_pick_index(self):
        if self.idx is None:
            raise AttributeError("Missing INDEX! " + "Run the work method first!")
        #
        if isinstance(self.idx, int):
            return self.idx
        else:
            raise TypeError("INDEX is not an integer: %s" % str(type(self.idx)))

    def get_pick(self):
        if self.idx is None:
            raise AttributeError("Missing PICK! " + "Run the work method first!")
        else:
